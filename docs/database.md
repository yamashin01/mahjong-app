...
ランキング機能は、データベースビューを使用せず、アプリケーション側で `game_results` テーブルから直接集計します。

### 3.1 イベントランキング

**実装場所**: `src/lib/supabase/repositories/rankings.ts` - `getEventRankings()`

イベント単位でのランキングを計算します：
- イベントに紐づく全対局の `game_results` を集計
- プレイヤーごとに総ポイント、対局数、順位分布を計算
- 同点の場合は同順位として扱う

**表示場所**: イベント詳細ページ (`/groups/[id]/events/[eventId]`)

### 3.2 削除されたビュー

以下のビューは未使用のため削除されました（マイグレーション `029_drop_unused_views.sql`）：
- `daily_rankings` - 日別ランキングビュー
- `group_statistics` - グループ統計ビュー
...
### クリーンアップ
28. `029_drop_unused_views.sql` - 未使用のビュー削除（daily_rankings, group_statistics）
