...
interface EditGroupDescriptionProps {
  groupId: string;
  currentDescription: string | null;
}

export function EditGroupDescription({ groupId, currentDescription }: EditGroupDescriptionProps) {
  ...
  const handleSave = async () => {
    ...
    const result = await updateGroupDescription(formData);
      ...
      <div className="space-y-2 w-full">
        <div className="flex flex-col gap-2 w-full">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500">{value.length}/500文字</span>
  ...
  return (
    <div className="group flex items-start gap-2">
      <p className="text-gray-600 flex-1">{currentDescription || "説明なし"}</p>
      <button
        type="button"
